...
import org.apache.spark.ml.feature.{IndexToString, StringIndexer, VectorIndexer}
    ...
    val assembler = new VectorAssembler()
      ...
      .setOutputCol("userFea")

    val userProfile = assembler
      .transform(data.na.fill(-1e9))
      .select("label","userFea")

    val data_train = userProfile
      .na
      .fill(-1e9)
    ...
    .fit(userProfile)
    val featureIndexer = new VectorIndexer().setInputCol("userFea").setOutputCol("indexedFeatures").setMaxCategories(4)
    .fit(userProfile)
...
import org.apache.spark.mllib.util.MLUtils
  ...
  //重点在这：用ML的VectorAssembler构建的vector，必须要有这个格式的转换，
  // 从ML的vector转成 MLlib的vector，才能给MLlib里面的分类器使用(这两种vector还真是个坑，要注意)
  val userProfile2 = MLUtils.convertVectorColumnsFromML(userProfile, "userFea")

  // 取训练样本 (userProfile2 is a dataframe)
  ...
  val maxBins = 32
  val numTrees = 70
  modelRF = RandomForest.trainClassifier(rdd_Data, 
                                          numClasses,
